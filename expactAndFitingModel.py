...
train1_DOW_MeanStdSumAllsum['DOW_powaer_rate'] = train1_DOW_MeanStdSumAllsum.DOW_power_sum / train1_DOW_MeanStdSumAllsum.DOW_allsum
...
train1_collist = list(groupbydataset.columns)
for i in range(66,78):
    train1_collist.remove(i)
train1_collist.remove('user_id')
train1_collist.remove('day_of_week')
train1_dayofweek_dataset = groupbydataset.drop(train1_collist, axis=1)
train1_dayofweek_matrix = train1_dayofweek_dataset.drop(['user_id', 'day_of_week'], axis=1).as_matrix()
...
train1_mergeDataset_add.drop(['DOW_power_sum', 'DOW_allsum', 'power_sum', 'DOW_powaer_rate', 'power_rate'], axis=1,inplace=True)

train1_Y = handledataset[(handledataset.record_date>=(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*82, unit='D'))) & (handledataset.record_date<(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*83, unit='D')))]

final_train1 = pd.merge(train1_mergeDataset_add,train1_Y,on=['user_id', 'day_of_week'], how='left')

#final_train1_matrix = final_train1.drop(['user_id', 'day_of_week', 'record_date'], axis=1).as_matrix()
#train1_dataset = final_train1_matrix[:(int(len(final_train1_matrix)*0.9)), :]
#test1_dataset = final_train1_matrix[(int(len(final_train1_matrix)*0.9)):, :]
#X_train1 = train1_dataset[:,:-1]
#Y_train1 = train1_dataset[:,-1]
#X_test1 = test1_dataset[:,:-1]
#Y_test1 = test1_dataset[:,-1]
...
train2_DOW_MeanStdSumAllsum['DOW_powaer_rate'] = train2_DOW_MeanStdSumAllsum.DOW_power_sum / train2_DOW_MeanStdSumAllsum.DOW_allsum
...
#    return group.T
# groupbydataset = handledataset.groupby(['user_id', 'day_of_week']).apply(test).reset_index()
train2_collist = list(groupbydataset.columns)
for i in range(67,79):
    train2_collist.remove(i)
train2_collist.remove('user_id')
train2_collist.remove('day_of_week')

train2_mergeDataset_add = pd.merge(train2_mergeDataset,groupbydataset.drop(train2_collist, axis=1),on=['user_id', 'day_of_week'], how='left')
...
train2_mergeDataset_add.drop(['DOW_power_sum', 'DOW_allsum', 'power_sum', 'DOW_powaer_rate', 'power_rate'], axis=1,inplace=True)

train2_Y = handledataset[(handledataset.record_date>=(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*83, unit='D'))) & (handledataset.record_date<(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*84, unit='D')))]

final_train2 = pd.merge(train2_mergeDataset_add,train2_Y,on=['user_id', 'day_of_week'], how='left')

#final_train2_matrix = final_train2.drop(['user_id', 'day_of_week', 'record_date'], axis=1).as_matrix()
#train2_dataset = final_train2_matrix[:(int(len(final_train2_matrix)*0.9)), :]
#test2_dataset = final_train2_matrix[(int(len(final_train2_matrix)*0.9)):, :]
#X_train2 = train2_dataset[:,:-1]
#Y_train2 = train2_dataset[:,-1]
#X_test2 = test2_dataset[:,:-1]
#Y_test2 = test2_dataset[:,-1]
...
train3_DOW_MeanStdSumAllsum['DOW_powaer_rate'] = train3_DOW_MeanStdSumAllsum.DOW_power_sum / train3_DOW_MeanStdSumAllsum.DOW_allsum
...
#    return group.T
# groupbydataset = handledataset.groupby(['user_id', 'day_of_week']).apply(test).reset_index()
train3_collist = list(groupbydataset.columns)
for i in range(68,80):
    train3_collist.remove(i)
train3_collist.remove('user_id')
train3_collist.remove('day_of_week')

train3_mergeDataset_add = pd.merge(train3_mergeDataset,groupbydataset.drop(train3_collist, axis=1),on=['user_id', 'day_of_week'], how='left')
...
train3_mergeDataset_add.drop(['DOW_power_sum', 'DOW_allsum', 'power_sum', 'DOW_powaer_rate', 'power_rate'], axis=1,inplace=True)

train3_Y = handledataset[(handledataset.record_date>=(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*84, unit='D'))) & (handledataset.record_date<(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*85, unit='D')))]
...
train4_DOW_MeanStdSumAllsum['DOW_powaer_rate'] = train4_DOW_MeanStdSumAllsum.DOW_power_sum / train4_DOW_MeanStdSumAllsum.DOW_allsum
...
#    return group.T
# groupbydataset = handledataset.groupby(['user_id', 'day_of_week']).apply(test).reset_index()
train4_collist = list(groupbydataset.columns)
for i in range(69,81):
    train4_collist.remove(i)
train4_collist.remove('user_id')
train4_collist.remove('day_of_week')

train4_mergeDataset_add = pd.merge(train4_mergeDataset,groupbydataset.drop(train4_collist, axis=1),on=['user_id', 'day_of_week'], how='left')
...
train4_mergeDataset_add.drop(['DOW_power_sum', 'DOW_allsum', 'power_sum', 'DOW_powaer_rate', 'power_rate'], axis=1,inplace=True)

train4_Y = handledataset[(handledataset.record_date>=(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*85, unit='D'))) & (handledataset.record_date<(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*86, unit='D')))]
...
test_DOW_MeanStdSumAllsum['DOW_powaer_rate'] = test_DOW_MeanStdSumAllsum.DOW_power_sum / test_DOW_MeanStdSumAllsum.DOW_allsum
...
#    return group.T
# groupbydataset = handledataset.groupby(['user_id', 'day_of_week']).apply(test).reset_index()
test_collist = list(groupbydataset.columns)
for i in range(70,82):
    test_collist.remove(i)
test_collist.remove('user_id')
test_collist.remove('day_of_week')

test_mergeDataset_add = pd.merge(test_mergeDataset,groupbydataset.drop(test_collist, axis=1),on=['user_id', 'day_of_week'], how='left')
...
test_mergeDataset_add.drop(['DOW_power_sum', 'DOW_allsum', 'power_sum', 'DOW_powaer_rate', 'power_rate'], axis=1,inplace=True)

test_Y = handledataset[(handledataset.record_date>=(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*86, unit='D'))) & (handledataset.record_date<(pd.to_datetime('2015-01-01') + pd.to_timedelta(7*87, unit='D')))]

test = pd.merge(test_mergeDataset_add,test_Y,on=['user_id', 'day_of_week'], how='left')
...
final_test_matrix = test.drop(['user_id', 'day_of_week', 'record_date'], axis=1).as_matrix()
test_X = final_test_matrix[:,:-1]
test_Y = final_test_matrix[:,-1]
...
if input_flag == 'y':
	joblib.dump(xgb_model, "train_xgb_model.m")
print "predict ............"
...
for i in range(len(test_Y)):
    if (predict_Y[i]+test_Y[i]) == 0:
        continue
    summ+=abs((predict_Y[i]-test_Y[i])/(predict_Y[i]+test_Y[i]))
meansum = summ/len(test_Y)
print "MeanSum = ",meansum

print "corrcoef = ",np.corrcoef(predict_Y, test_Y)
